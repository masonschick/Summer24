...
######################################################################### initializes a browser session to a given url (basic exception handling)
...
class ArbFinder(object):
    ...
    def __init__(self, URL):
        try:
            ...
            self.driver.implicitly_wait(5)               ############### makes Selenium wait for elements before throwing exception
            self.driver.get(URL)
        except:
            pass
        self.sport = 'Tennis'

########################################################################### configures scraper as ASK or BID
...
############################################################################ creates betting parameters and initializes 'ArbFinder' for both books
...
class App(object):
    def __init__(self):
        ...
        self.ignored_exceptions = (NoSuchElementException, StaleElementReferenceException,)  ############### here down is ignored
...
######################################################################################## starts / stops applications main loop
...
######################################################################################## handles different stages of the betting process (finding live odds --> placing bets)
...
################################################################################ handles trading logic using nash equilibrium for optimal bets
...
####################################################################### Sets up the GUI and binds actions to buttons
